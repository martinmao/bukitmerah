...
public class Servlets {
	...
	/**
	 * 取得带相同前缀的Request Parameters,保持原有类型并包含前缀
	 * 
	 * @param request
	 * @param prefixs
	 * @return
	 */
	public static Map<String, String[]> getParametersStartingWith(ServletRequest request, String[] prefixs) {
	...
	public static Map<String, Object> getParametersStartingWith(ServletRequest request, String prefix,
		...
		while ((paramNames != null) && paramNames.hasMoreElements()) {
			...
			if ("".equals(prefix) || paramName.startsWith(prefix)) {
				String unprefixed = keepPrefix ? paramName : paramName.substring(prefix.length());
	...
	/**
	 * The returned URL contains a protocol, server name, port number, and path,
	 * but it does not include query string parameters.
	 * 
	 * @param request
	 * @param append
	 *            path or querys append to current request url.
	 * @return
	 */
	public static URL fromCurrentRequestUrl(HttpServletRequest request, String append) {
		StringBuffer current = request.getRequestURL();
		String url;
		if (StringUtils.hasText(append)) {
			if (append.charAt(0) == PATH_SEPARATOR && current.charAt(current.length() - 1) == PATH_SEPARATOR) {
				current.deleteCharAt(current.length() - 1);
			} else if (append.charAt(0) != PATH_SEPARATOR && current.charAt(current.length() - 1) != PATH_SEPARATOR) {
				current.append(PATH_SEPARATOR);
			}
			url = current.append(append).toString();
		} else
